...
const registerController = async (req, res) => {
    ...
    try{
        const { name, username, emailId, password, confirmPassword, phoneNumber, address } = req.body;

        // Fields are Empty
        if(!name || !username || !emailId || !password || !confirmPassword || !phoneNumber || !address){
        ...
        const existingEmail = await adminModel.findOne({emailId});
        if(existingEmail){
            return res.status(200).send({
                success: false,
        ...
        // Save
        const user = await new adminModel({ name, username, slug: slugify(username), emailId, password : hashedPassword, phoneNumber, address });
...
const loginController = async (req, res) => {
    ...
    try{
        ...
        const user = await adminModel.findOne({ $or: [ { username: credentials }, { emailId: credentials } ] });
        if(!user){
            return res.status(200).send({
                success: false,
...
const updateProfileController = async (req, res) => {
    ...
    try{
        ...
        const updateUser = await adminModel.findByIdAndUpdate(req.user._id, {
...
export { registerController, loginController, logoutController, updateProfileController };
